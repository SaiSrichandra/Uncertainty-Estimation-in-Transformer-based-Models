...
class EncoderLayer(nn.Module):
    ...
    """
    def __init__(self, attention, d_model, d_ff=None, moving_avg=25, dropout=0.1, activation="relu", configs=None,lnum = None):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.configs.is_training == False and self.configs.noiseFF == True and self.configs.afterNorm == True and self.lnum == self.configs.layerNum:
            ...
            res = self.configs.add_noise(res ,self.configs.noiseType,self.configs.nParam_mu,self.configs.nParam_sigma,self.configs.nParam_rate)
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.conv_layers is not None:
            ...
            for attn_layer in self.attn_layers:
                x, attn = attn_layer(x, attn_mask=attn_mask)
                # if self.configs.noiseType == 'dropout':
                #     x = self.configs.add_noise(x,self.configs.noiseType,self.configs.nParam_mu,self.configs.nParam_sigma,self.configs.nParam_rate)
                attns.append(attn)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, self_attention, cross_attention, d_model, c_out, d_ff=None,
                 moving_avg=25, dropout=0.1, activation="relu", configs = None, lnum = None):
